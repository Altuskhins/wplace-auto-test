...
// @version      2.1
// @description  Автоматически ставит пиксели в заданной области на wplace.live, используя токен капчи пользователя и отправляя уведомления. Исправлена проблема с CSP.
// @author       Gemini
...
(function() {
    ...
    /**
     * Эта функция внедряет основной код скрипта на страницу,
     * обходя защиту Content-Security-Policy (CSP) сайта.
     */
    function injectScriptWithNonce() {
        // 1. Находим 'nonce' (ключ-разрешение) из уже существующих на странице скриптов.
        const nonce = document.querySelector('script[nonce]')?.nonce;

        if (!nonce) {
            console.error("[Auto-Farm] Не удалось найти CSP nonce. Скрипт не может быть запущен.");
            alert("Ошибка запуска Auto-Farm: Не удалось найти токен безопасности (nonce) для запуска скрипта.");
            return;
        }

        // 2. Создаем новый элемент <script>.
        const script = document.createElement('script');
        script.setAttribute('nonce', nonce); // 3. Устанавливаем найденный nonce.

        // 4. Весь основной код бота помещается внутрь этого скрипта как текст.
        script.textContent = `
        (async () => {
          // --- НАСТРОЙКИ ---
          const CONFIG = {
        ...
        // 5. Добавляем скрипт на страницу для выполнения и сразу удаляем, чтобы не засорять DOM.
        document.body.appendChild(script).remove();
    }

    // Ждем, пока DOM будет готов, прежде чем внедрять скрипт.
    if (document.body) {
        injectScriptWithNonce();
    } else {
        window.addEventListener('DOMContentLoaded', injectScriptWithNonce);
    }
})();
